...
serverless deploy -v
export S3_BUCKET=$(severless info -v | grep BucketSiteName | sed s/BucketSiteName\:\ //g | awk '{print $1}')
export CLOUDFRONT_DISTRIBUTION_ID=$(severless info -v | grep CloudFrontDistributionId | sed s/CloudFrontDistributionId\:\ //g | awk '{print $1}')
